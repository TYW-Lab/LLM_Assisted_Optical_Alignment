...
class OpticalSystem(Optic):
    def __init__(self,
                 ...
                 mirror1_position: np.ndarray = np.array([0, 0, 10.0]),
                 mirror2_position: np.ndarray = np.array([0, 10, 10.0]),
                 rotation_angles_mirror1: np.ndarray = np.array([np.pi/4, 0.0, 0.0]),  # [rx1, ry1, rz1]
                 rotation_angles_mirror2: np.ndarray = np.array([np.pi/4, 0.0, 0.0]),  # [rx2, ry2, rz2]
                 aperture_dist: float = 100.0,
        ...
        aperture_dist : float, default 100.0 Distance between aperture 1 and aperture 2 (in mm).
        m2_a1_dist : float, default 20.0 Distance between mirror 2 and aperture 1 (in mm).
        a1_d1_dist : float, default 10.0 Distance between aperture 1 and detector 1 (in mm).
        a2_d2_dist : float, default 10.0 Distance between aperture 2 and detector 2 (in mm).
        """
        ...
        self.aperture1_position = calculate_position(p2_hit, d2_out, m2_a1_dist)
        self.aperture2_position = calculate_position(p2_hit, d2_out, m2_a1_dist + aperture_dist)
        ...
        self.add_surface(
            index=5,
            x=self.aperture1_position[0],
            z=self.aperture1_position[2],
            y=self.aperture1_position[1],
        ...
        self.add_surface(
            index=8,
            x=self.aperture2_position[0],
            z=self.aperture2_position[2],
            y=self.aperture2_position[1],
            rx=rotation_angles[0],
            ry=rotation_angles[1],
            rz=rotation_angles[2],
            aperture=ap_detector1,
    ...
    def set_mirror_angle(self, rx1=None, ry1=None, rx2=None, ry2=None, gradient: bool = False):
        ...
        if gradient == True:
            ...
            cs4.rx = rx2
            cs4.ry = ry2
            # Detector On Mirror 2 should follow mirror2's rotation
            cs3.rx = rx2
            cs3.ry = ry2
    
    def normal_direction(self,R,n=np.array([0,0,1])):
        """Calculate the normal vector of mirror"""
        n_global = R @ n
        n_global = n_global / np.linalg.norm(n_global)
        return n_global

    def set_mirror_angle_delta(self, delta_rx1=0, delta_ry1=0, delta_rx2=0, delta_ry2=0, gradient: bool = False):

        """
        Set the rotation delta of mirror 1 and mirror 2. The unit is rad.
        """
        ...
        def _adjust_angle(a, delta):
            if a is None or delta is None:
                return a
            if isinstance(a, torch.Tensor):
                delta = torch.tensor(delta, dtype=a.dtype, device=a.device)
                return (a + delta) % (2 * torch.pi)
            else:
                return (a + delta ) % (2 * np.pi)
        
        rx1 = _adjust_angle(rx1, delta_rx1) if delta_rx1 != 0 else rx1
        ry1 = _adjust_angle(ry1, delta_ry1) if delta_ry1 != 0 else ry1
        rx2 = _adjust_angle(rx2, delta_rx2) if delta_rx2 != 0 else rx2
        ry2 = _adjust_angle(ry2, delta_ry2) if delta_ry2 != 0 else ry2
